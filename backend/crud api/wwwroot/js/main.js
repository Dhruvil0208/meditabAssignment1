...
//code to get patient by id
// the url in browser window should be like below
//http://localhost:5076/?id=57
...
function GetCall() {
    if (id != null) {
        fetch(`http://localhost:5076/Patient/${id}`, {
            method: 'GET',
            headers: {
            ...
            .then(data => {
                ...
                if (data.length !=0) {
                    ...
                    document.getElementById('lname1').value = data[0].last_name;
                    document.getElementById('mname').value = data[0].middle_name;
                    document.getElementById('gender').value = data[0].sex;
                    document.getElementById('img-txt').innerHTML = data[0].chart_id;
                    document.getElementById('dob').value = data[0].dob.substr(0, 10);
...
//if the "id" is passed in the url than update is triggered else new patient is created
// this function is called when submit button is clicked
function PostPutApiCall() {
    ...
    let url = 'http://localhost:5076/Patient/';
    ...
    fetch(url,
        ...
        .then(data => {
            ...
            const patient_id = data[0].patientcreate;
            alert("patient created successfully");
            if (methodType == "POST") {
                window.location.replace(`${window.location}?id=${patient_id}`);
            }
...
let loadFile = function(event){
    ...
    if(event.target.files.length > 0){
        profileImg.src = URL.createObjectURL(event.target.files[0]);
        document.getElementsByClassName("remove-img")[0].style.visibility = "visible";
    }
    
}

let removeFile = function(event){
    profileImg.src = 'styles\istockphoto-1331257339-1024x1024.jpg';
    document.getElementsByClassName("remove-img")[0].style.visibility = "hidden";
}
