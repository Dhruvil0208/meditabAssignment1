...
namespace crud_api.Controllers
{
    ...
    public class PatientController : Controller
    {
        ...
        public JsonResult patientgetbyid(int id)
        {
            ...
            string query = @"
               select * from search_patient(patientId=>@id)
            ";
...
/// <returns></returns>
        ...
        public JsonResult Put( int id,Patient p)
        {
            string query = @"select patientupdate(@id,@FirstName,@LastName,@MiddleName,@Sex,@Dob::date)";
...
*/
        ...
        /// <returns>list of patients based on the input given </returns>
       
        [HttpPost("search")]
        public JsonResult Post(GetPatient gp)
        {
            string query = @"
                select * from search_patient(@pageNumber::int,@pageSize::int,@firstName,@lastName,@gender,@dob::date,@orderBy,@sortOrder,@patientId
                )	
            ";
            ...
            using (NpgsqlConnection myCon = new NpgsqlConnection(sqlDataSource))
            {
                ...
                using (NpgsqlCommand myCommand = new NpgsqlCommand(query, myCon))
                {
                    ...
                    //sort order
                    if (gp.sortOrder != null && gp.sortOrder != "")
                    {
                        myCommand.Parameters.AddWithValue("@sortOrder", gp.sortOrder);
                    }
                    else
                    {
                        myCommand.Parameters.AddWithValue("@sortOrder", DBNull.Value);
                    }
                    //Patient check
                    if (gp.patientId != null )
                    {
                        myCommand.Parameters.AddWithValue("patientId", gp.patientId);
                    }
                    else
                    {
                        myCommand.Parameters.AddWithValue("@patientId", DBNull.Value);
                    }
